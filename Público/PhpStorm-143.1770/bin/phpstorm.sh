...
# Try (in order): WEBIDE_JDK, ../jre, JDK_HOME, JAVA_HOME, "java" in PATH.
# ---------------------------------------------------------------------
...
if [ ! -r "$vm_options_file" ]; then
  vm_options_file="$OS_SPECIFIC_BIN_DIR/phpstorm$BITS.vmoptions"
fi
user_vm_options_file="$HOME/.WebIde100/phpstorm$BITS.vmoptions"
if [ -r "$user_vm_options_file" ]; then
  vm_options_file="$user_vm_options_file"
fi
if [ -n "$WEBIDE_VM_OPTIONS" ] && [ -r "$WEBIDE_VM_OPTIONS" ]; then
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=WebIde100"
  fi
fi
...
LD_LIBRARY_PATH="$IDE_BIN_HOME:$LD_LIBRARY_PATH" "$JAVA_BIN" \
  ...
  -Djb.restart.code=88 -Didea.paths.selector=WebIde100 \
...
RESTARTER="$HOME/.WebIde100/restart/restarter.sh"
if [ -x "$RESTARTER" ]; then
  "$RESTARTER"
  "$RM" -f "$RESTARTER"
fi
